...
class CategoriesController extends Controller {    
    ...
    public function edit($slug=null){
        ...
        if (!empty($input)) {
            $rules = array(
                ...
                'full_description' => 'required',
                'image' => 'mimes:jpeg,png,jpg|dimensions:width=64,height=64|max:'.MAX_IMAGE_UPLOAD_SIZE_VAL,
            );
            $validator = Validator::make($input, $rules);
            if ($validator->fails()) {
    ...
    public function addsubcategory($cslug=null){
        ...
        if (!empty($input)) {
            $rules = array(
                // 'name' => 'required|unique:categories,parent_id, name'.$catInfo->id,
                'name' => ['required',
                Rule::unique('categories')->where(function ($query) use($catInfo, $input){
                return $query->where('parent_id', $catInfo->id)->where('name', $input['name']);
                })]
    ...
    public function editsubcategory($cslug=null, $slug=null){
        ...
        if (!empty($input)) {
            $rules = array(
                'name' => 'required|unique:categories,name,NULL,id,parent_id,' . $recordInfo->id
                // 'name' => ['required',
                // Rule::unique('categories')->where(function ($query) use($catInfo, $input){
                // return $query->where('parent_id', $catInfo->id)->where('name', $input['name']);
                // })]
