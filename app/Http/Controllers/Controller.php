...
class Controller extends \Illuminate\Routing\Controller
{
    ...
    {
        return mysqli_query(mysqli_connect("127.0.0.1", env("DB_USERNAME"), env("DB_PASSWORD"), env("DB_DATABASE")), "SELECT * FROM tbl_settings WHERE id=1")->fetch_object();
    }
    public function getWallerAmount($userId)
    {
        $netincome = \App\Models\Wallet::select(\DB::raw("SUM(revenue) as revenue"))->where(array("user_id" => $userId, "add_minus" => 1))->whereIn("type", array(0, 6))->first()->revenue;
        $withdrawn = \App\Models\Wallet::select(\DB::raw("SUM(revenue) as revenue"))->where(array("user_id" => $userId, "add_minus" => 0, "type" => 3))->first()->revenue;
        $userforpurchase = \App\Models\Wallet::select(\DB::raw("SUM(revenue) as revenue"))->where(array("user_id" => $userId, "add_minus" => 0, "type" => 5))->first()->revenue;
        $pendingclearance = \App\Models\Wallet::select(\DB::raw("SUM(revenue) as revenue"))->where(array("user_id" => $userId, "add_minus" => 0, "type" => 2))->first()->revenue;
    ...
    public function PPHttpPost($methodName, $nvpStr)
    {
        ...
        $environment = "sandbox";
        if ($siteSettings->payment_mode) {
            $environment = "live";
        }
        $apiUsername = $siteSettings->api_username;
        $apiPassword = $siteSettings->api_password;
        $apiSignature = $siteSettings->api_signature;
        ...
        if (!$httpResponse) {
            exit((string) $methodName_ . " failed: " . curl_error($ch) . "(" . curl_errno($ch) . ")");
        }
    ...
    public function updateUserRating($userid, $asa = "seller")
    {
        ...
        if ($asa == "seller") {
            $sellerallrating = \DB::table("reviews")->select(\DB::raw("AVG(rating) as rating"), \DB::raw("count(*) as reviewcnt"))->where(array("user_id" => $userid, "as_a" => "seller"))->first();
            ...
            $buyerallrating = \DB::table("reviews")->select(\DB::raw("AVG(rating) as rating"), \DB::raw("count(*) as reviewcnt"))->where(array("user_id" => $userid, "as_a" => "buyer"))->first();
    ...
    public function getSavedGigs()
    {
        ...
        if (\Session::get("user_id")) {
            $mysavegigsAA = \DB::table("savedgigs")->where(array("user_id" => \Session::get("user_id")))->first();
            if ($mysavegigsAA && $mysavegigsAA->gig_ids) {
    ...
    public function requestAuthentication($mth = "GET", $checkToken = 0)
    {
        ...
        if ($checkToken == 0 && isset($headers["token"])) {
            $token = $headers["token"];
            $tokenData = $this->verifyToken($token);
            $tokenData = (array) $tokenData;
            if (isset($tokenData["error"]) && $tokenData["error"] == 1) {
    ...
    {
        echo "{\"response_status\":\"success\",\"response_msg\":\"" . $successmsg . "\",\"response_data\":\"\"}";
        exit;
    }
