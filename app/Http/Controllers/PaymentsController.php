...
class PaymentsController extends Controller {
    ...
    public function __construct() {
     ...
     public function createPaypalPayment($slug = null)
    {   
       $pageTitle = 'Payment With PayPal';
        ...
        $ppId = $settingsInfo->api_username;
        $ppSecret = $settingsInfo->api_password;


       $apiContext = new \PayPal\Rest\ApiContext(
        new \PayPal\Auth\OAuthTokenCredential(
            $ppId,     // ClientID
            $ppSecret     // ClientSecret
        )
        );
        ...
        $items[0] = new Item();
        $items[0]->setName($recordInfo->Gig->title)
            ->setCurrency($currency)
            ->setQuantity(1)
            // ->setSku($slug) // Similar to `item_number` in Classic API
            ->setPrice($totalAmt);
        ...
        $redirectUrls->setReturnUrl("http://laravel.seenshop.com/payments/executePaypal")
            ->setCancelUrl("http://laravel.seenshop.com/payments/paypalcancel/" .$slug);
    ...
    public function executePaypal(Request $request, $slug = null){
        ...
        $mc_gross = $sale->getAmount();
        $amountPaid = $mc_gross->getTotal();
        $slug = $transactions[0]->getDescription();
    ...
    public function addtocart(Request $request){ 
        ...
        $amtname = $type.'_price';
        if($request->has('gigprice')){
            $amount = $request->get('gigprice');
        } else {
...
//echo '<pre>';print_r($transactionId);exit;
    ...
    public function history(Request $request){ 
        ...
        $allrecords  = Payment::with('Gig')->with('service')->where('user_id', Session::get('user_id'))->orderBy('id', 'DESC')->get();
        // dd($allrecords);
