<template>
  <div>
      <Contacts :onlineUsers="onlineUsers" :receiver="receiver" :user="user"/>
                        <Messages :onlineUsers="onlineUsers" :usersTyping="usersTyping" :user="user" :receiver="receiver" />
  </div>
</template>
...
export default {
    ...
    mounted(){
        ...
        Echo.private(`typingTo.${this.user.id}`)
                ...
                .listenForWhisper('typingStop', (user) => {
                        this.typingUsers = this.typingUsers.filter(u => u.id != user.id);
                })    
