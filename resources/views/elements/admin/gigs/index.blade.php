                    ...
                    <tbody>
                        @foreach($allrecords as $allrecord)
                            <tr>
                                ...
                                <td data-title="Name">{{$allrecord->title}}</td>
                                <td data-title="Seller Name">{{ isset($allrecord->User->first_name) ? $allrecord->User->first_name.' '.$allrecord->User->last_name : '' }}</td>
                                <td data-title="Category Name">{{$allrecord->Category->name or ''}}</td>
                                ...
                                <td data-title="Action">
                                    ...
                                    <span class="right_acdc" id="status{{$allrecord->id}}">
                                        @if($allrecord->status == '1')
                                            <a href="{{ URL::to( 'admin/gigs/deactivate/'.$allrecord->slug)}}" title="Deactivate" class="deactivate"><button class="btn btn-success btn-xs"><i class="fa fa-check"></i></button></a>
                                        @else
                                            <a href="{{ URL::to( 'admin/gigs/activate/'.$allrecord->slug)}}" title="Activate" class="activate"><button class="btn btn-danger btn-xs"><i class="fa fa-ban"></i></button></a>
                                        @endif
                                    </span>
<!--                                    <a href="{{ URL::to( 'admin/gigs/edit/'.$allrecord->slug)}}" title="Edit" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
                                    <a href="{{ URL::to( 'admin/gigs/delete/'.$allrecord->slug)}}" title="Delete" class="btn btn-danger btn-xs action-list delete-list" onclick="return confirm('Are you sure you want to delete this record?')"><i class="fa fa-trash-o"></i></a>-->
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
