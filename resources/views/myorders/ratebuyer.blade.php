        ...
        <div class="workplace">
            ...
            <div class="management-bx">
                <div class="message_chat">
                    ...
                    @if($myorderInfo->status == 2 && $myorderInfo->is_seller_rate != 1)
                       ...
                       <div class="comment-form reviewrate">
                        ...
                        {{ Form::open(array('method' => 'post', 'id' => 'ratebuyerrform')) }}
                        ...
                        <div class="giverate">
                            <div class="giverate_str">
                                ...
                                {{Form::hidden('rating', 0, ['class'=>'form-control required', 'id'=>'selectrating'])}}
                    ...
                    @if($myorderInfo->buyer_id == Session::get('user_id'))
                        ...
                        <div class="dpimg-about_right">
                            <div class="buy_row">
                                ...
                                <div class="buy_name">
                                    <span><a href="{{ URL::to( 'public-profile/'.$myorderInfo->Seller->slug)}}">{{$myorderInfo->Seller->user_name}}</a></span>
                    ...
                    @else
                       <div class="req_dtl_head">About the Buyer</div>
                        <div class="dpimg-about_right">
                            <div class="buy_row">
                                <div class="buy_img">
                                    @if($myorderInfo->Buyer->profile_image)
                                    <a href="{{ URL::to( 'public-profile/'.$myorderInfo->Buyer->slug)}}">{{HTML::image(PROFILE_SMALL_DISPLAY_PATH.$myorderInfo->Buyer->profile_image, SITE_TITLE, ['id'=> 'pimage'])}}</a>
                                    @else
                                    <a href="{{ URL::to( 'public-profile/'.$myorderInfo->Buyer->slug)}}">{{HTML::image('public/img/front/user-img.png', SITE_TITLE, ['id'=> 'pimage'])}}</a>
                                    @endif
                                </div>
                                <div class="buy_name">
                                    <span><a href="{{ URL::to( 'public-profile/'.$myorderInfo->Buyer->slug)}}">{{$myorderInfo->Buyer->user_name}}</a></span>
                                    <div class="about-rating">
                                        <script>
                                            $(function() {
                                                $('#avgRating222').raty({
                                                    starOn:    'star-on.png',
                                                    starOff:   'star-off.png',
                                                    start: {{$myorderInfo->Buyer->average_rating}},
                                                    readOnly: true
                                                });
                                            });
                                        </script>
                                        <span class="pprate" id="avgRating222"></span>
                                        <span class="rating-view"><b>{{$myorderInfo->Buyer->average_rating}}</b> ({{$myorderInfo->Buyer->total_review}} reviews)</span>
                                    </div>
                                 </div>
                            </div>
                            <div class="buy_row">
                                <ul class="general-info">
                                            <li>
                                                <label><i class="fa fa-map-marker" aria-hidden="true"></i>From</label>
                                                <span>@isset($myorderInfo->Buyer->Country->name) {{$myorderInfo->Buyer->Country->name}} @endisset</span>
                                            </li>
                                            <li>
                                                <label><i class="fa fa-user" aria-hidden="true"></i>Member since</label>
                                                <span>{{$myorderInfo->Buyer->created_at->format('M Y')}}</span>
                                            </li>
                                            <li>
                                                <label><i class="fa fa-user" aria-hidden="true"></i>Languages</label>
                                                <span>@if($myorderInfo->Buyer->languages)
                                                        @foreach(json_decode($myorderInfo->Buyer->languages) as $key => $lang)                                                      
                                                            @if(!$loop->first), @endif{!!$lang->lang_name!!}
                                                        @endforeach
                                                      @else 
                                                        N/A
